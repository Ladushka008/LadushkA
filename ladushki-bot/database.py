...
async def update_balance(user_id: int, delta: int) -> int:
    """
    Атомарно изменяет баланс на delta (может быть положительным или отрицательным).
...
async def claim_daily_bonus(user_id: int) -> Tuple[bool, int, Optional[timedelta]]:
    ...
    Возвращает (успех, полученная сумма, оставшееся время до следующего).
    """
    now = datetime.now()
    async with aiosqlite.connect(Config.DB_FILE) as db:
...
async def use_item(user_id: int, item_name: str) -> bool:
    """Списывает 1 предмет из инвентаря, если он есть."""
    async with aiosqlite.connect(Config.DB_FILE) as db:
        async with db.execute(
